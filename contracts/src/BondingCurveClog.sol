...
contract BondingCurveClog is ReentrancyGuard {
    ...
    address public immutable eligibilityRegistry; // notified on every successful trade (see
        // `_touchEligibility`) -- the same tokenId used everywhere else (TickerNFT, this
        // contract's own `tickerTokenId`) identifies this meme to EligibilityRegistry too.
    ...
    constructor(
        ...
        require(
            token_ != address(0) && tickerNFT_ != address(0) && multisig_ != address(0)
                && winnerPot_ != address(0) && governance_ != address(0) && eligibilityRegistry_ != address(0),
            "zero address"
        );
    ...
    /// @dev Notifies EligibilityRegistry that this token was just traded, so it can update
    ///      `aboveThresholdSince`/qualification state as a normal, automatic side effect of
    ///      trading (see EligibilityRegistry.onTrade). Called as the LAST step of both buy() and
    ///      sell(), after every state update, tax route, and event.
    ///
    ///      ATOMIC BY DESIGN, NOT BEST-EFFORT: this call is a normal, unwrapped external call --
    ///      if EligibilityRegistry reverts, the whole trade reverts with it. This is deliberate,
    ///      not an oversight: a best-effort/try-catch version was considered and rejected. The
    ///      concrete failure it introduces: a sell that drops real reserve below threshold, whose
    ///      touch call happens to fail and gets silently swallowed, would leave
    ///      `aboveThresholdSince` at its OLD pre-dip value -- the registry would never learn the
    ///      streak was actually broken, and a later successful touch (once reserve recovers) would
    ///      see a nonzero `aboveThresholdSince` and leave it untouched rather than resetting it,
    ///      letting the token qualify off a streak that was never actually continuous. Silently
    ///      losing exactly the touch that would have recorded a below-threshold dip is the one
    ///      failure mode this system cannot tolerate, since `_touch` only writes a reset in its
    ///      below-threshold branch -- a missed call there is indistinguishable, from the
    ///      registry's perspective, from a dip that never happened. Requiring every trade to
    ///      succeed together with its own eligibility touch removes that failure mode by
    ///      construction: either both the trade and the touch land, or neither does, so real
    ///      market state and eligibility state can never diverge. EligibilityRegistry is a
    ///      trusted, protocol-owned, immutable contract (not an arbitrary or adversarial address
    ///      like a ticket-owner recipient could be), and the revert surface of `onTrade` for any
    ///      properly-launched, registered token is empty under normal operation -- every "already
    ///      qualified" / "duplicate candidate" / "no active round" case it could hit resolves via
    ///      an early return, never a revert (see the audit in the accompanying report). No bounded
    ///      gas stipend is needed either: this is a trusted protocol call, not a payment to a
    ///      potentially adversarial recipient, so ordinary transaction gas is the right model, the
    ///      same as any other internal call this contract makes to itself or its own dependencies.
