...
loop:
    ledger.scanForNewEvents()   # incremental reconstruction of existing outstanding round work

    # 1. qualify matured tokens BEFORE closing a due round
    #    so a token that matured during this round can enter this round
    ...
    # 2. close due rounds only after qualification has had its chance
