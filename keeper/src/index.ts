 ...
 * deliberate: scan the ledger for already-existing settlement state, then
 * qualify matured tokens BEFORE closing a due round so a token that becomes
 * eligible during the current round is included in that same round rather
 * than being pushed into the next one. Only after qualification do we close
 * due rounds (which can also trigger the first randomness request attempt),
 * then retry any randomness requests that failed earlier, relay any results
 * Arbitrum has already fulfilled, and run the observational checks last. Every
