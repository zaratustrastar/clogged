...
export async function checkFundingHealth(config: KeeperConfig, clients: Clients): Promise<{ level: "info" | "warn"; detail: string }[]> {
  ...
  const keeperAddress = clients.keeperAddress;
  const [providerBalance, wrapperBalance, keeperRobinhoodBalance, keeperArbitrumBalance] = await Promise.all([
    ...
    keeperAddress ? clients.robinhoodPublic.getBalance({ address: keeperAddress }) : Promise.resolve(undefined),
    keeperAddress ? clients.arbitrumPublic.getBalance({ address: keeperAddress }) : Promise.resolve(undefined),
  ]);
  ...
  if (keeperAddress && keeperRobinhoodBalance !== undefined && keeperArbitrumBalance !== undefined) {
    checks.push(
      { label: `Keeper EOA (${keeperAddress}) ETH on Robinhood Chain`, balance: keeperRobinhoodBalance },
      { label: `Keeper EOA (${keeperAddress}) ETH on Arbitrum One`, balance: keeperArbitrumBalance }
    );
  } else {
    // Neither KEEPER_PRIVATE_KEY nor the optional, public KEEPER_ADDRESS
    // was available (dry-run with neither set) - the keeper EOA doesn't
    // exist yet to have a balance at all, so this is skipped rather than
    // guessed at or defaulted to a zero balance. Provider/Wrapper/VRF
    // subscription funding (checked above and below) are entirely
    // independent of the keeper EOA and are still checked regardless.
    results.push({
      level: "info",
      detail: "Keeper EOA balance checks skipped: no KEEPER_PRIVATE_KEY and no KEEPER_ADDRESS set. Supply KEEPER_ADDRESS to see this in --dry-run without a signing secret.",
    });
  }
